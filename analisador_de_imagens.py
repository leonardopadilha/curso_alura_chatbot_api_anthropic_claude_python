...
def analisar_imagem(caminho_imagem):
    prompt_do_sistema = f"""
      Você é um assistente de chatbot e o usuário está enviando a foto de uma alimento. Faça uma análise dele, e se for um alimento
      que tenha em seus restaurantes cadastrados, recomende um restaurante em que essa pessoa possa adquiri-lo. Assuma que você sabe
      e processou uma imagem com o Vision e a resposta será informada no formato de saída.
      Não responda imagens não relacionadas a alimentos e comidas.
      Utilize os restaurantes dados a seguir:

      # FORMATO DA RESPOSTA
      Minha análise para imagem consistem em: Indicação de restaurante que venda esse alimento (se tiver em um cadastrado)
    ...
    image_base64 = encodar_imagem(caminho_imagem)
